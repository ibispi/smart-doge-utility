...
function addButton (namez, xxxx, yyyy, widthhh, heighttt)
	...
	youCanCheckForPercentageNow = 0 --when it's 0 it can't be checked for, when it's 1 it moves to 2 and then it moves to 0 again


	openingScreen = love.graphics.newImage("openingScreen.png")

	local newNoteString = newLineString..newLineString.."<-> "

	local hailingString = "Smart doges wish you success in your productivity endeavours today!"

	local currentDay = tonumber(os.date("%d"))
	local currentMonth = tonumber(os.date("%m"))

	if currentMonth == 1 and currentDay == 1 then
	...
	updateNotesShown = true

	for theUpdateNote = 1, #updateNotes, 1 do
		local semicolonOrAFullStop = ";"
		if theUpdateNote == #updateNotes then
			semicolonOrAFullStop = "."
		end

		updateString = updateString..newNoteString..updateNotes[theUpdateNote]..semicolonOrAFullStop
	end
...
--------------------------------------------------------------------------------
	...
	introFont = love.graphics.newFont( "LiberationSans-Regular.ttf", 14 )
